...
# [Problem Set 6 Module]
    ...
    mu.zero(dim,1)
    mu.value[0][0] = world_size/2.0
    mu.value[1][0] = world_size/2.0
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                mu.value[b][0] += -measurement[i][1+b]/measurement_noise
                mu.value[m+b][0] += measurement[i][1+b]/measurement_noise

        #squeezing space for the new movement
        idxs=[0,1]+list(range(4,dim+2))
        Omega = Omega.expand(dim+2,dim+2,idxs,idxs)
        mu = mu.expand(dim+2,1,idxs,[0])
        for b in range(4):
            Omega.value[b][b] += 1.0/motion_noise
        for b in range(2):
            Omega.value[b][b+2] += -1.0/motion_noise
            Omega.value[b+2][b] += -1.0/motion_noise
            mu.value[b][0] += -motion[b]/motion_noise
            mu.value[b+2][0] += motion[b]/motion_noise
        ...
        mu = mu.take(newidxs,[0]) - a.transpose()*b.inverse()*c
