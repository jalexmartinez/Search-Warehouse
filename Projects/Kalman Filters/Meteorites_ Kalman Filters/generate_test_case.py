...
INITIAL_LASER_STATE = {"h": np.pi * 0.5, "hp": 50, "laser_shots_remaining": 50}

TURRET_INITIAL_POS = {"x": 0.0, "y": -1.0}
...
class MeteorShowerGenerator(object):
    ...
    def generate(
        ...
        def meteorite_init_location_bad(coes):
            ...
            return (
                (xypos[0] <= arena.x_bounds[0])
                or (xypos[0] >= arena.x_bounds[1])
                or (xypos[1] < y_init_bounds[0])
                or (xypos[1] > y_init_bounds[1])
            )
            # the last condition here is to prevent meteorites from
            # generating very close to the ground

        # Generate meteorites from t_past timesteps to t_future timesteps every
        # t_step timesteps
        for t in range(t_past, t_future):
            if not (t % t_step):
                regenerate = True
                while regenerate:
                    coeffs = self.generate_meteorite_coeffs(
                        overall_x_pos_bounds,
        ...
        return MeteorShower(
            arena, self.seed, P_HIT, LASER_LENGTH, meteorites, turret, min_dist
        )
...
def params(args):
    ...
    dummy_turret = Turret(
        {"x": TURRET_INITIAL_POS["x"], "y": TURRET_INITIAL_POS["y"]},
        my_args.max_angle_change,
        my_args.dt,
    )

    field = field_generator.generate(
        t_past=my_args.t_past,
        ...
        y_init_bounds=(my_args.min_y_init, my_args.max_y_init),
    )

    return {
        "meteorites": [dict(a.params) for a in field.meteorites],
...
def main(args):
    ...
    # remove save location info that json file doesn't need internally
...
def parser():
    ...
    prsr.add_argument(
        "--turret_hp", help="Turret's initial health point count", type=int, default=50
    )
    prsr.add_argument(
        "--num_laser_shots",
    ...
    prsr.add_argument(
        "--t_past",
        help="timestep in past (negative integer) from which to start generating meteorites",
        type=int,
        default=-100,
    )
    prsr.add_argument(
        "--t_future",
        help="timestep into future (positive integer) at which to stop generating meteorites",
        type=int,
        default=600,
    )
    prsr.add_argument(
        "--t_step", help="add a meteorite every N-th time step", type=int, default=4
    )
    prsr.add_argument(
        "--noise_sigma_x",
    ...
    prsr.add_argument(
        "--nsteps", help="Number of timesteps to simulate", type=int, default=400
    )
    prsr.add_argument(
        "--dt", help="Duration of a single timestep", type=float, default=0.1
    )
    prsr.add_argument(
        "--meteorite_c_pos_max",
    ...
    prsr.add_argument(
        ...
        type=float,
        default=0.0873,  # this is ~5 degrees in radians
    )
    prsr.add_argument(
        "--seed",
