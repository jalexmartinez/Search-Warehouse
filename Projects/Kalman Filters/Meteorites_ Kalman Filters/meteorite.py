...
class MeteorShower(object):
    ...
    def laser_or_ground_hit(self, time, laser_heading_rad,
        ...
        for meteorite in self.meteorites:
            if meteorite.id < 0:
                continue
            # was this meteorite in-bounds last timestep?
            prev_xyloc = meteorite.xy_pos(time-0.1)
            # check for ground hits
            if self.arenacontains((prev_xyloc[0], prev_xyloc[1])) and \
                    meteorite.xy_pos(time)[1] < self.turret.y_pos and \
                    meteorite.t_start <= time:
